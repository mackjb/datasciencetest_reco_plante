    ...
    if task == 'species':
        ...
        if args.splits_file and os.path.isfile(args.splits_file):
            ...
            if not (tr_p and va_p and te_p):
                ...
                tr_y = [1 if disease_from_path(p) != 'healthy' else 0 for p in tr_p]
                va_y = [1 if disease_from_path(p) != 'healthy' else 0 for p in va_p]
                te_y = [1 if disease_from_path(p) != 'healthy' else 0 for p in te_p]
        else:
            tr_p, tr_y, va_p, va_y, te_p, te_y = stratified_splits(all_files, labels_all, seed=args.seed)
    else:  # disease
        ...
        if args.splits_file and os.path.isfile(args.splits_file):
            with open(args.splits_file, 'r', encoding='utf-8') as f:
                sp = json.load(f)
            print(f"[INFO] Loaded splits from: {args.splits_file} (filtered to diseased only)")
            eli_set = set(diseased_files)
            def _keep(seq):
                return [p for p in seq if p in eli_set]
            tr_p = _keep(sp.get('train', []))
            va_p = _keep(sp.get('val', []))
            te_p = _keep(sp.get('test', []))
            if not (tr_p and va_p and te_p):
