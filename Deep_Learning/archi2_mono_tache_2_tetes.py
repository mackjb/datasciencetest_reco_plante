...
# -*- coding: utf-8 -*-
"""
mono_tache_2_tetes.py — Architecture 3: CNN mono-objectif - 2 têtes de sortie

2 runs indépendants (pas simultanés):
  1) species     — Quelle est cette plante ? (14 classes)
  2) disease_all — Quelle est la condition ? (21 classes: 20 maladies + healthy)

Différence avec mono_tache.py:
- Seulement 2 tâches au lieu de 3 (pas de health binaire)
- disease_all traite "healthy" comme une classe à part entière (pas de filtrage)
...
def write_report_md(output_dir, class_names, class_counts_df, best_metrics, fig_paths, task_name):
    ...
    lines = [
        f"# Architecture 3: CNN mono-objectif - 2 têtes | Task: {task_name}\n",
        ...
        f"- Classification report: {os.path.relpath(fig_paths['classification_report'], output_dir)}\n",
...
def main():
    parser = argparse.ArgumentParser(description="Architecture 3: CNN mono-objectif - 2 têtes (species/disease_all)")
    parser.add_argument('--task', type=str, choices=['species','disease_all'], default='species',
                        help='species: 14 classes | disease_all: 21 classes (20 maladies + healthy)')
    parser.add_argument('--data_root', type=str, required=True)
    parser.add_argument('--output_dir', type=str, default='outputs_mono_2heads')
    ...
    if task == 'species':
        ...
        # Charger splits communs si fournis
        if args.splits_file and os.path.isfile(args.splits_file):
            ...
            if not (tr_p and va_p and te_p):
                print("[WARN] Loaded splits incomplete; falling back to task-stratified splits.")
                tr_p, tr_y, va_p, va_y, te_p, te_y = stratified_splits(all_files, labels_all, seed=args.seed)
            else:
                tr_y = [name2idx[species_from_path(p)] for p in tr_p]
                va_y = [name2idx[species_from_path(p)] for p in va_p]
                te_y = [name2idx[species_from_path(p)] for p in te_p]
        else:
            tr_p, tr_y, va_p, va_y, te_p, te_y = stratified_splits(all_files, labels_all, seed=args.seed)
    else:  # disease_all
        # Toutes les images: disease + healthy comme classes (pas de filtrage)
        names = sorted({disease_from_path(fp) for fp in all_files})
        name2idx = {n:i for i,n in enumerate(names)}
        idx2name = {i:n for n,i in name2idx.items()}
        labels_all = [name2idx[disease_from_path(fp)] for fp in all_files]
        if args.splits_file and os.path.isfile(args.splits_file):
            with open(args.splits_file, 'r', encoding='utf-8') as f:
                sp = json.load(f)
            print(f"[INFO] Loaded splits from: {args.splits_file}")
            ...
            if not (tr_p and va_p and te_p):
                ...
                tr_y = [name2idx[disease_from_path(p)] for p in tr_p]
                va_y = [name2idx[disease_from_path(p)] for p in va_p]
                te_y = [name2idx[disease_from_path(p)] for p in te_p]
        else:
            tr_p, tr_y, va_p, va_y, te_p, te_y = stratified_splits(all_files, labels_all, seed=args.seed)
