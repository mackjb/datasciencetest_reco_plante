...
# -*- coding: utf-8 -*-
"""
multi_tache_2_tetes_health_est_une_classe.py — Multi‑tâche (species + disease_all)

Architecture simplifiée avec 2 têtes simultanées:
  1) species     — Quelle est cette plante ? (14 classes)
  2) disease_all — Quelle est la condition ? (21 classes: 20 maladies + healthy)

Différences avec multi_tache_2_tetes.py:
- Pas de tête health binaire ni de filtrage
- healthy est traitée comme la 21ème classe de disease_all
- Tous les échantillons entraînent les deux têtes (pas de sample_weight)
- Architecture simplifiée sans tête auxiliaire interne

- Backbone: EfficientNetV2S (ImageNet), gelé puis fine‑tuning partiel
- Gradient clipping disponible pour stabilité
- tf.data pipeline, prétraitement 0..255
- Rapport + matrices + courbes
"""
...
def make_datasets_2heads(paths: Tuple[List[str], List[str], List[str]],
    ...
    def encode(fp: tf.Tensor):
        # tf.py_function wrapper to build labels
        # Architecture 8: PAS de sample_weight, tous les échantillons contribuent
        def _py(fp_obj):
            ...
            dis = disease_from_path(p)  # Peut être "healthy" ou une maladie
            sp_idx = s2i[sp]
            y_sp = keras.utils.to_categorical(sp_idx, num_classes=len(s2i)).astype(np.float32)
            # Disease: healthy est une classe normale (pas de traitement spécial)
            dis_idx = d2i[dis]  # Inclut healthy
            y_dis = keras.utils.to_categorical(dis_idx, num_classes=len(d2i)).astype(np.float32)
            # Tous les échantillons ont un poids de 1.0
            w_sp = 1.0
            w_dis = 1.0
        ...
        y_dis.set_shape((len(d2i),))  # 21 classes
        w_sp.set_shape(())
        w_dis.set_shape(())
...
def build_model_2heads(n_species: int, n_disease_all: int, img_size=(256,256), initial_lr=1e-3, weight_decay=1e-4,
                       label_smoothing_species=0.1, label_smoothing_disease=0.1):
    """
    Architecture 8: healthy EST une classe de disease (21 classes)
    - Pas de tête health auxiliaire
    - Disease+Healthy directement depuis les features du backbone
    - Tous les échantillons contribuent à l'entraînement (pas de sample_weight)
    """
    ...
    # Species head (logits in float32 for stability under mixed precision)
    sp_logits = layers.Dense(n_species, activation='softmax', dtype='float32', name='species')(x)

    # Disease+Healthy head: 21 classes directement depuis features (pas de tête auxiliaire)
    dis_all_logits = layers.Dense(n_disease_all, activation='softmax', dtype='float32', name='disease')(x)

    model = keras.Model(inputs, {'species': sp_logits, 'disease': dis_all_logits})
...
class MacroF1TwoCallback(keras.callbacks.Callback):
    """Architecture 8: Calcule macro‑F1 pour species et disease (disease inclut healthy comme classe)."""
    def __init__(self, val_images_ds, val_species_idx, val_disease_idx):
        ...
        self.y_dis = np.array(val_disease_idx)  # Tous >= 0 (healthy est une classe)
    def on_epoch_end(self, epoch, logs=None):
        from sklearn.metrics import f1_score, accuracy_score
        preds = self.model.predict(self.val_images_ds, verbose=0)
        p_sp = np.argmax(preds['species'], axis=1)
        p_dis = np.argmax(preds['disease'], axis=1)  # Toutes les 21 classes
        logs = logs or {}
        # Species metrics
        ...
        # Disease+Healthy metrics (21 classes)
        f1_dis = f1_score(self.y_dis, p_dis, average='macro')
        acc_dis = accuracy_score(self.y_dis, p_dis)
        logs['val_disease_macro_f1'] = float(f1_dis)
        logs['val_disease_accuracy_sklearn'] = float(acc_dis)
        print(f"\n[VAL] Epoch {epoch+1}: species F1={f1_sp:.4f}, disease+healthy F1={f1_dis:.4f} (21 classes)")
...
def plot_training_curves_2heads(history_df, out_path):
...
def write_report_md(output_dir, species_names, disease_all_names, best_metrics, fig_paths, report_paths=None):
    p = os.path.join(output_dir, 'report.md')
    lines = [
        f"# Architecture 8: Multi‑tâche (species, disease+healthy)\n",
        f"Date: {datetime.now().isoformat()}\n\n",
        f"## Label spaces\n",
        f"- #species: {len(species_names)}\n",
        f"- #diseases+healthy: {len(disease_all_names)} (INCLUT healthy)\n\n",
        ...
        f"- Disease+Healthy (21 classes): Val Acc={best_metrics.get('val_acc_disease','N/A')}, Val F1={best_metrics.get('val_f1_disease','N/A')}, Test Acc={best_metrics.get('test_acc_disease','N/A')}, Test F1={best_metrics.get('test_f1_disease','N/A')}\n\n",
    ...
    lines.extend([
        ...
        f"- Confusion matrix (disease+healthy, 21 classes): {os.path.relpath(fig_paths['cm_disease'], output_dir)}\n",
...
def main():
    parser = argparse.ArgumentParser(description="Multi‑tâche 2 têtes PlantVillage (species/disease)")
    parser.add_argument('--data_root', type=str, required=True)
    parser.add_argument('--output_dir', type=str, default='outputs_multi_2heads')
    ...
    # Architecture 8: healthy EST une classe de disease (21 classes total)
    disease_all_names = sorted({disease_from_path(fp) for fp in all_files})  # Inclut healthy
    if not disease_all_names:
        raise RuntimeError("No disease samples found.")
    s2i = {s:i for i,s in enumerate(species_names)}
    d2i = {d:i for i,d in enumerate(disease_all_names)}  # Inclut healthy
    i2s = {i:s for s,i in s2i.items()}
    i2d = {i:d for d,i in d2i.items()}
    print(f"[INFO] Architecture 8: Disease classes = {len(disease_all_names)} (includes healthy)")
    ...
    with open(os.path.join(args.output_dir, 'diseases_all.json'), 'w', encoding='utf-8') as f:
        json.dump(disease_all_names, f, indent=2, ensure_ascii=False)
    print(f"[INFO] Saved disease_all (21 classes including healthy) to diseases_all.json")
    ...
    train_ds, val_ds, test_ds, val_x, test_x = make_datasets_2heads(
    ...
    def build_indices(paths: List[str]):
        ...
        # Architecture 8: healthy est une classe normale (pas de -1)
        dis_idx = [d2i[disease_from_path(p)] for p in paths]  # Tous >= 0
        return sp_idx, dis_idx
    va_sp, va_dis = build_indices(va_p)
    te_sp, te_dis = build_indices(te_p)

    # Modèle
    macro_cb = MacroF1TwoCallback(val_images_ds=val_x, val_species_idx=va_sp, val_disease_idx=va_dis)
    ...
    if args.report_only:
        ...
        model, base, loss_dict, metrics_dict = build_model_2heads(
            n_species=len(s2i), n_disease_all=len(d2i), img_size=tuple(args.img_size),
    ...
    # Évaluation (val/test)
    # Architecture 8: Évaluer sur TOUTES les 21 classes (healthy inclus)
    from sklearn.metrics import f1_score, accuracy_score, confusion_matrix, classification_report
    # VAL
    pv = model.predict(val_x, verbose=0)
    p_sp_v = np.argmax(pv['species'], axis=1)
    p_dis_v = np.argmax(pv['disease'], axis=1)  # 21 classes
    f1_sp_v = f1_score(va_sp, p_sp_v, average='macro')
    acc_sp_v= accuracy_score(va_sp, p_sp_v)
    f1_dis_v= f1_score(va_dis, p_dis_v, average='macro')  # Sur les 21 classes
    acc_dis_v=accuracy_score(va_dis, p_dis_v)
    
    # TEST
    pt = model.predict(test_x, verbose=0)
    p_sp_t = np.argmax(pt['species'], axis=1)
    p_dis_t = np.argmax(pt['disease'], axis=1)  # 21 classes
    f1_sp_t = f1_score(te_sp, p_sp_t, average='macro')
    acc_sp_t= accuracy_score(te_sp, p_sp_t)
    f1_dis_t= f1_score(te_dis, p_dis_t, average='macro')  # Sur les 21 classes
    acc_dis_t=accuracy_score(te_dis, p_dis_t)

    # Confusion matrices
    cm_sp = confusion_matrix(te_sp, p_sp_t, labels=list(range(len(s2i))))
    cm_dis = confusion_matrix(te_dis, p_dis_t, labels=list(range(len(d2i))))  # Toutes les 21 classes
    ...
    # Disease+Healthy report (21 classes)
    cr_dis = classification_report(te_dis, p_dis_t, target_names=[i2d[i] for i in range(len(d2i))], digits=4)
    cr_dis_path = os.path.join(args.output_dir, 'classification_report_disease_all.txt')
    with open(cr_dis_path, 'w', encoding='utf-8') as f:
        f.write(cr_dis)
    print(f"[INFO] Saved disease+healthy classification report (21 classes): {cr_dis_path}")

    # Sauvegardes
    cm_sp_path  = os.path.join(args.output_dir, 'cm_species.png')
    cm_dis_path = os.path.join(args.output_dir, 'cm_disease_all.png')
    plot_confusion_matrix(cm_sp, [i2s[i] for i in range(len(s2i))], cm_sp_path)
    plot_confusion_matrix(cm_dis, [i2d[i] for i in range(len(d2i))], cm_dis_path)  # 21 classes

    curves_path = os.path.join(args.output_dir, 'training_curves.png')
    try:
        if 'hist' in locals() and hist is not None:
            plot_training_curves_2heads(hist, curves_path)
        else:
            hist2 = None
            if os.path.exists(os.path.join(args.output_dir, 'history.csv')):
                hist2 = pd.read_csv(os.path.join(args.output_dir, 'history.csv'))
            if hist2 is not None:
                plot_training_curves_2heads(hist2, curves_path)
            else:
    ...
    report_paths = {
        ...
        'cr_disease': cr_dis_path,  # Architecture 8: 21 classes
    }
    write_report_md(args.output_dir, species_names, disease_all_names, best_metrics, fig_paths, report_paths)
