...
# -*- coding: utf-8 -*-
"""
Logistic Regression (multinomial, saga) sur PlantVillage.
...
3) Pipeline: Suppression NA (en amont) -> RobustScaler -> LogisticRegression(saga, elasticnet)
4) CV 5-fold stratifiée avec métriques (balanced_accuracy, f1_macro)
5) Grid search hyperparamètres (C, l1_ratio), refit sur f1_macro
...
- NA: suppression si NA pour la cible, exclusion des lignes avec `is_na=True` si la colonne existe, puis suppression des lignes avec NA sur les colonnes features.
- Doublons: si colonne `is_duplicate_after_first` présente, ceux flaggés True sont exclus (même logique que les CSV "clean").
- RobustScaler est préféré à StandardScaler à cause des outliers.
"""
...
RESULTS_DIR: Path = PROJECT_ROOT / "results" / "models" / "logreg_multinomial_saga"
...
def print_plan() -> None:
    ...
    steps = [
        ...
        "2) Split stratifié train/test",
        "3) Pipeline: Suppression NA features -> RobustScaler -> LogisticRegression(saga, elasticnet)",
...
def build_pipeline(random_state: int = RANDOM_STATE) -> Pipeline:
    lr = LogisticRegression(
        solver='saga',
        penalty='elasticnet',  # l1_ratio sera tuné
        l1_ratio=0.5,
        C=1.0,
    ...
    # Ordre: sampler -> scaler -> model (le scaler est ajusté après échantillonnage)
    pipe = ImbPipeline(steps=[
        ("sampler", "passthrough"),  # sera configuré par GridSearch (passthrough, ROS, SMOTE)
...
def tune_hyperparams(pipe: Pipeline, X: pd.DataFrame, y: pd.Series, cv_splits: int = 5, random_state: int = RANDOM_STATE) -> GridSearchCV:
    ...
    # Comparer class_weight vs oversampling (ROS/SMOTE) sans double correction
    param_grid = [
        {
            "sampler": ["passthrough"],
        ...
        },
        {
            ...
            "model__class_weight": [None],  # éviter sampler + balanced simultanément
            "model__C": np.logspace(-3, 2, 6),
            "model__l1_ratio": [0.0, 0.25, 0.5, 0.75, 1.0],
        },
    ]
...
def summarize_and_plot(cv_results_df: pd.DataFrame) -> None:
    ...
    # Heatmap (moyenne bal_acc) par (C, l1_ratio) facettée par approche
    try:
        ...
        # Si heatmap échoue (ex: manque de colonnes), ignorer silencieusement
        with open(plots_dir / "plot_errors.log", "a") as f:
            f.write(f"Heatmap error: {e}\n")

    # Heatmap (moyenne f1_macro) par (C, l1_ratio) facettée par approche
    try:
