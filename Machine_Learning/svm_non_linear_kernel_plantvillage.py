...
def build_pipeline(random_state: int = RANDOM_STATE) -> Pipeline:
    """StandardScaler -> SVC(kernel='rbf'). On tune C et gamma."""
    pipe = Pipeline([
        ("scaler", StandardScaler()),  # sera overridé par la grille (Standard vs Robust)
        ("svm", SVC(
            kernel="rbf",
            cache_size=1000,
...
def main() -> None:
    ...
    # Grille rapide pour confirmer le plateau autour de C≈80
    ...
    # Export JSON: meilleurs hyperparamètres + métriques test pour réutilisation
    try:
        svm = best_pipe.named_steps.get("svm")
        scaler = best_pipe.named_steps.get("scaler")
        export = {
            "model": "SVC",
            "scaler": scaler.__class__.__name__ if scaler is not None else None,
            "best_params_cv": {
                k: (str(v) if k == "scaler" else v) for k, v in gs.best_params_.items()
            },
            "svm_params": {
                k: v for k, v in svm.get_params().items() if k in [
                    "kernel", "C", "gamma", "class_weight", "decision_function_shape", "random_state"
                ]
            } if svm is not None else None,
            "test_metrics": {
        ...
        with open(RESULTS_DIR / "best_run.json", "w", encoding="utf-8") as f:
            json.dump(export, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print(f"Avertissement: échec export best_run.json: {e}")
