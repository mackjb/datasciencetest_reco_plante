...
def render_dl_page():
    ...
    with tab_archi3:
        ...
        for i, ex in enumerate(examples_archi3):
            with cols[i]:
                ...
                if st.button(f"Sélect. {ex['id']}", key=f"btn3_{ex['id']}"):
                    ...
                    st.session_state.analyzed3 = False
                    st.rerun()
                st.image(ex["img_orig"], width="stretch")
        ...
        with c1:
            st.markdown("#### 📥 Entrée")
            st.image(selected3["img_orig"], caption="Image originale", width="stretch")

        with c2:
            st.markdown("<br><br><br>", unsafe_allow_html=True)
            if st.button("🚀 Lancer l'Analyse Archi 3", type="primary", disabled=st.session_state.analyzed3):
        ...
        with c3:
            ...
            if st.session_state.analyzed3:
                st.image(selected3["img_cam"], caption="Grad-CAM (zones influentes)", width="stretch")
            else:
    ...
    with tab_archi9:
        ...
        with c1:
            st.markdown("#### 📥 Entrée")
            st.image(selected9["img_orig"], caption="Image originale", width="stretch")

        with c2:
            st.markdown("<br><br><br>", unsafe_allow_html=True)
            if st.button("🚀 Lancer l'Analyse Archi 9", type="primary", disabled=st.session_state.analyzed9):
                with st.spinner("Inférence en cours..."):
                    time.sleep(1.2)
        ...
        with c3:
            st.markdown("#### 🔍 Interprétation")
            if st.session_state.analyzed9:
                st.image(selected9["img_cam"], caption="Grad-CAM (zones influentes)", width="stretch")
            else:
                st.info("Lancez l'analyse pour visualiser la carte de chaleur.")
