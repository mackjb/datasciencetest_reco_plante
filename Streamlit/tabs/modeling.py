    ...
    with tabs[2]:
        ...
        **Observations clés :**
        ...
        if os.path.exists(p1):
            st.image(p1, caption="Importance Globale des Features (Top 25)", width="stretch")
        else:
...
def render_dl_content():
    ...
    with st.expander("👥 Phase d'Exploration Individuelle", expanded=True):
        ...
        st.dataframe(df_models_t, width="stretch")
            ...
            with st.expander(f"Architecture {arch['num']} : {arch['nom']}"):
                ...
                with col2:
                    if os.path.exists(arch['img']):
                        st.image(arch['img'], caption=f"Schéma Architecture {arch['num']}", width="stretch")
