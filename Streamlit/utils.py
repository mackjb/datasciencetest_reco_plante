    ...
    <script>
        ...
        async function render() {{
            ...
            try {{
                ...
                // Wait for container to have dimensions before initializing PanZoom
                // This prevents 'matrix not invertible' errors if the tab is hidden/animating
                let attempts = 0;
                const initPanZoom = setInterval(() => {{
                    if ((graphDiv.clientWidth > 0 && graphDiv.clientHeight > 0) || attempts > 50) {{
                        clearInterval(initPanZoom);
                        try {{
                             // Setup PanZoom
                            ...
                            // Initial Zoom Step (Custom preference) - Zoom out slightly for margin
                            panZoomInstance.zoomBy(0.85); 
                            ...
                            console.error("PanZoom Initialization Error:", pzErr);
                        }}
                    }}
                    attempts++;
                }}, 100);
