...
def duplicate_dataset_limited(src_dir, dst_dir, max_files_per_class=5):
    ...
    for root, dirs, files in os.walk(src_dir):
        ...
        image_files = sorted(image_files)[:max_files_per_class]
...
if __name__ == "__main__":
    ...
    dst = project_root / "dataset" / "new_plant_diseases_dataset" / "data"
    if dst.exists():
        print(f"⚠️  Le dataset existe déjà à : {dst}")
    else:
        download_path = Path(kagglehub.dataset_download("vipoooool/new-plant-diseases-dataset"))
        print("Path to dataset files:", download_path)
        move_dataset_if_exists(download_path, dst)
        # Trouver le dossier racine du dataset (contenant 'train' et 'valid')
        train_dirs = list(dst.rglob('train'))
        if train_dirs:
            src_dataset = train_dirs[0].parent
        else:
            src_dataset = dst
        print(f"✅ Dataset racine détecté : {src_dataset}")
        dst_dataset = dst / f"{src_dataset.name}_5images"
