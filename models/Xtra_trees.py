...
RESULTS_DIR: Path = PROJECT_ROOT / "results_modifiés" / "models" / "extra_trees_baseline"
...
def build_xtr_pipeline(random_state: int = RANDOM_STATE) -> Pipeline:
...
def plot_validation_curves_xtr(method_name: str, pipe: Pipeline, X, y, best_params: Dict[str, float], results_dir: Path, n_splits: int = 3) -> Dict[str, object]:
    out: Dict[str, object] = {}
    cv = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=42)

    n_est_values = np.array([200, 400, 600])
    train_scores, test_scores = validation_curve(
        pipe, X, y,
        param_name="xtr__n_estimators",
    ...
    mf_values = np.array([0.5, 0.75, 1.0])
    pipe_mf = Pipeline(pipe.steps)
    pipe_mf.set_params(**{"xtr__n_estimators": int(best_params.get("xtr__n_estimators", 300))})
    train_scores_g, test_scores_g = validation_curve(
        pipe_mf, X, y,
        param_name="xtr__max_features",
        param_range=mf_values,
        scoring="f1_macro",
        cv=cv,
        n_jobs=1,
    )
    ...
    fig2 = px.line(x=mf_values, y=test_mean_g, error_y=test_std_g, labels={"x": "max_features", "y": "F1_macro (CV)"}, title=f"Validation curve — max_features — {method_name}")
    fig2.add_scatter(x=mf_values, y=train_mean_g, mode="lines", name="Train")
    p2 = results_dir / "plots" / method_name / "validation_curve_max_features.html"
    p2.parent.mkdir(parents=True, exist_ok=True)
    pio.write_html(fig2, str(p2), auto_open=False)
    out["validation_curve_max_features_path"] = p2
...
def save_diagnostics(method_name: str, results_dir: Path, cv_rep: Dict[str, Dict[str, float]], bootstrap: Dict[str, Dict[str, float]], complexity: Dict[str, float], valcurves: Optional[Dict[str, object]]) -> None:
    ...
    diag = {
        ...
        "validation_curve_paths": {
            ...
            "max_features": str(valcurves.get("validation_curve_max_features_path")) if valcurves else None,
        },
    }
    (method_dir / "diagnostics.json").write_text(json.dumps(diag, ensure_ascii=False, indent=2), encoding="utf-8")


def compute_xtr_complexity(fitted_pipe: Pipeline) -> Dict[str, float]:
    xtr = fitted_pipe.named_steps.get("xtr")
    if xtr is None:
        return {"n_estimators": float("nan"), "avg_tree_depth": float("nan")}
    depths = []
    try:
        for est in getattr(xtr, "estimators_", []):
            depths.append(float(getattr(est.tree_, "max_depth", np.nan)))
    except Exception:
        pass
    avg_depth = float(np.nanmean(depths)) if depths else float("nan")
    return {
        "n_estimators": float(getattr(xtr, "n_estimators", float("nan"))),
        "avg_tree_depth": avg_depth,
    }


def main() -> None:
    print("\nPlan d'évaluation (Extra Trees Baseline):")
    for s in [
        "1) Charger CSV + nettoyer",
        "2) Split stratifié train/test",
        "3) RobustScaler -> ExtraTrees: CV + test",
    ...
    pipe = build_xtr_pipeline(RANDOM_STATE)
    grid = {
        "xtr__n_estimators": [300],
        "xtr__class_weight": [None, "balanced"],
    }
    gs, cv_df = run_grid_cv(pipe, X_train, y_train, grid, RESULTS_DIR / "xtr_grid.csv")
    best = gs.best_estimator_
    test_scores = evaluate_test(best, X_test, y_test)
    summary = cv_summary_str(cv_df, ["param_xtr__n_estimators", "param_xtr__class_weight"]).sort_values("param_xtr__n_estimators")

    _ = save_all_classif_artifacts("extra_trees", best, df, X_train, y_train, X_test, y_test)

    (RESULTS_DIR / "xtr_cv_summary.json").write_text(json.dumps({
        "best_params": gs.best_params_,
        "best_cv_f1_macro": float(gs.best_score_),
        "cv_rows": len(cv_df),
        "summary_preview": summary.head(10).to_dict(orient="records"),
    }, ensure_ascii=False, indent=2), encoding="utf-8")
    (RESULTS_DIR / "xtr_test.json").write_text(json.dumps(test_scores, ensure_ascii=False, indent=2), encoding="utf-8")

    print(f"XTR CV -> f1_macro: {gs.cv_results_['mean_test_f1_macro'][gs.best_index_]:.4f} ± {gs.cv_results_['std_test_f1_macro'][gs.best_index_]:.4f} | "
          f"bal_acc: {gs.cv_results_['mean_test_bal_acc'][gs.best_index_]:.4f} ± {gs.cv_results_['std_test_bal_acc'][gs.best_index_]:.4f}")
    print(f"XTR Test -> bal_acc: {test_scores['balanced_accuracy']:.4f} | f1_macro: {test_scores['f1_macro']:.4f}")
    ...
    complexity = compute_xtr_complexity(best)
    print(f"n_estimators = {complexity.get('n_estimators')} | avg_tree_depth = {complexity.get('avg_tree_depth')}")

    valcurves = plot_validation_curves_xtr("extra_trees", build_xtr_pipeline(RANDOM_STATE), X_train, y_train, {
        "xtr__n_estimators": gs.best_params_.get("xtr__n_estimators", 300)
