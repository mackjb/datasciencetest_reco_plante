#!/usr/bin/env python3
# Script pour explorer différents modèles pour la classification de maladies
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("automl_maladies.log"),
...
def load_data():
    """Charge les données pour la classification de maladies"""
    try:
...
def setup_experiment(data, target):
    """Configure l'expérience PyCaret pour la classification de maladies"""
    try:
        # Colonnes à ignorer (identifiants, chemins d'images, etc.)
        ignore_features = ['Image_Path', 'md5', 'nom_plante']
        ...
        exp = setup(
            ...
            experiment_name='maladies_classification',
            use_gpu=False
        )
...
def train_models():
    """Entraîne et évalue différents modèles pour la classification de maladies"""
    try:
        # Charger les données
        data = load_data()
        
        # Filtrer les échantillons sains (nous voulons uniquement les échantillons malades)
        data_maladies = data[data['nom_maladie'] != 'healthy'].copy()
        logging.info(f"Nombre d'échantillons de maladies: {len(data_maladies)}")
        
        # Configurer l'expérience pour la classification de maladies
        exp = setup_experiment(data_maladies, 'nom_maladie')
        ...
        os.makedirs('results/automl_maladies', exist_ok=True)
        ...
        print("\n" + "="*100)
        print("RÉSULTATS DE LA COMPARAISON DES MODÈLES (CLASSIFICATION DE MALADIES)")
        print("="*100)
        ...
        results_file = f'results/automl_maladies/model_comparison_{timestamp}.csv'
        results.to_csv(results_file, index=False)
        logging.info(f"Résultats détaillés sauvegardés dans {results_file}")
        
        # Sauvegarder le meilleur modèle
        best_model = best_models[0]
        model_file = f'results/automl_maladies/best_model_{timestamp}.pkl'
        ...
        summary_file = f'results/automl_maladies/summary_{timestamp}.json'
...
def main():
    try:
        logging.info("Début de l'exploration AutoML pour la classification de maladies")
        ...
        # Afficher un résumé des résultats
        print("\n" + "#" * 100)
        print("RÉSUMÉ DE L'ENTRAÎNEMENT - CLASSIFICATION DE MALADIES")
        print("#" * 100)
        ...
        for i, model in enumerate(summary['metriques'], 1):
            print(f"{i}. {model['Model']} - {summary['score_metrique']}: {model.get(summary['score_metrique'], 'N/A'):.4f}")
