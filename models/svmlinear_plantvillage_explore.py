...
def print_plan() -> None:
    ...
    steps = [
        ...
        "2) Split stratifié train/test",
        "3) Pipeline: StandardScaler -> LinearSVC(l2, squared_hinge, dual=False)",
        "4) GridSearchCV 5-fold uniquement sur C",
        "5) Évaluation finale test (balanced_accuracy, f1_macro, rapport, matrice de confusion)"
    ]
    for s in steps:
        print(" - ", s)
...
def build_pipeline(random_state: int = RANDOM_STATE) -> Pipeline:
    """StandardScaler -> LinearSVC. Tous les hyperparamètres sont fixés sauf C (tunable)."""
    pipe = Pipeline([
...
def save_feature_ranking(feature_names: List[str], best_estimator: Pipeline) -> Path:
    """Exporte un feature_ranking.csv basé sur les coefficients LinearSVC.
    Importance = moyenne des valeurs absolues des coefficients sur les classes (un-vs-rest).
    """
    svm = best_estimator.named_steps.get("svm")
    if svm is None or not hasattr(svm, "coef_"):
        raise RuntimeError("Aucun step 'svm' avec des coefficients n'a été trouvé dans le pipeline.")
...
def save_misclassified_examples(df_full: pd.DataFrame, X_test: pd.DataFrame, y_test: pd.Series, y_pred: np.ndarray,
    ...
    for col in ["filepath", "filename", "label", "species"]:
    ...
    if not df_err.empty:
        for (t, p), group in df_err.groupby(["true", "pred"]):
            ...
            for _, row in sample.iterrows():
                caption = f"true={row['true']} pred={row['pred']}"
                img_src = None
                ...
                if img_src:
                    lines.append(f"<div class='card'><img src='{img_src}' alt='{caption}'/><div>{caption}</div></div>")
                else:
                    lines.append(f"<div class='card'><div>Aucune image trouvée</div><div>{caption}</div></div>")
...
def build_error_table_with_margins(
    ...
    # Features standardisées pour contributions
    ...
    if svm is None or not hasattr(svm, "coef_"):
        raise RuntimeError("Le pipeline ne contient pas 'svm' avec coef_.")
    ...
    for r, idx in enumerate(idxs):
        ...
        # contributions topn vers la classe prédite
        contrib = z[r, :] * coef[ip, :]
        # top features par contribution décroissante
    ...
    eval_dir = RESULTS_DIR / "evaluation"
    eval_dir.mkdir(parents=True, exist_ok=True)
    out_csv = eval_dir / "errors_with_margins.csv"
    err_df.to_csv(out_csv, index=False)
    return err_df, out_csv
...
def main() -> None:
    ...
    # GridSearchCV uniquement sur C — grille resserrée autour du meilleur précédent
    Cs = np.array([50, 75, 100, 150, 200], dtype=float)
    ...
    # Sauvegarde et affichage de l'influence de C
    cv_df = pd.DataFrame(gs.cv_results_)
    cv_df.to_csv(RESULTS_DIR / "gridsearch_results.csv", index=False)
    plot_path = plot_c_influence(cv_df)
    print(f"Graphique Plotly sauvegardé: {plot_path}")
    # Reproduire la figure depuis le CSV sauvegardé (vérification)
    plot_from_csv = plot_c_influence_from_csv(RESULTS_DIR / "gridsearch_results.csv")
    print(f"Graphique Plotly (depuis CSV) sauvegardé: {plot_from_csv}")
    print("\nSynthèse CV par C (moyenne):")
    summary = (
        cv_df.assign(C=cv_df["param_svm__C"].astype(float))
             .sort_values("C")
             [["C", "mean_test_bal_acc", "mean_test_f1_macro", "rank_test_f1_macro"]]
    )
    print(summary.to_string(index=False, float_format=lambda v: f"{v:.4f}"))
    print(f"\nMeilleurs hyperparamètres: {gs.best_params_} | meilleur f1_macro (CV): {gs.best_score_:.4f}")
    ...
    # Génération et visualisation du feature ranking
