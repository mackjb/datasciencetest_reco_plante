...
# -*- coding: utf-8 -*-
"""
multi_tache_2_tetes.py — Entraînement multi‑tâche (species, disease) sur PlantVillage (segmented)
- 2 têtes de sortie: species (multi‑classe) et disease (multi‑classe hors "healthy")
- La notion de santé (healthy/diseased) n'est PAS une tête; elle est utilisée:
  1) comme feature interne pour la tête disease (probabilité d'être malade issue d'une tête auxiliaire interne)
  2) comme masque de perte: les échantillons healthy n'entraînent PAS la tête disease (sample_weight=0)
...
- Rapport + matrices de confusion + courbes d'entraînement
"""
...
def build_model_2heads(n_species: int, n_disease: int, img_size=(256,256), initial_lr=1e-3, weight_decay=1e-4,
    ...
    # Internal auxiliary health head -> feature for disease
    ...
    x_plus_h = layers.Concatenate(name='concat_health')([x, hl_prob_diseased])

    # Disease head uses base features + health feature
    dis_logits = layers.Dense(n_disease, activation='softmax', dtype='float32', name='disease')(x_plus_h)
...
class MacroF1TwoCallback(keras.callbacks.Callback):
    """Calcule macro‑F1 pour species et disease (disease: seulement sur malades)."""
