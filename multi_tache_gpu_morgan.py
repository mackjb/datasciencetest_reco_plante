...
def main():
    ...
    state_path = os.path.join(args.output_dir, 'training_state.json')
    ...
    if args.report_only:
        ...
        if args.resume and os.path.exists(state_path):
            with open(state_path, 'r') as f:
                state = json.load(f)
            print(f"[INFO] Resuming from: phase={state['phase']}, epoch={state['epoch']}")
            if state['phase'] == 1:
            ...
            history_csv_tmp = os.path.join(args.output_dir, 'history_phase1.csv')
            if os.path.exists(history_csv_tmp):
                hist_p1 = pd.read_csv(history_csv_tmp)
        
        # Phase 1: têtes seules
        if not skip_phase1:
            print(f"[INFO] Stage 1: Train heads (backbone frozen) - Starting from epoch {initial_epoch_p1}")
            h1 = model.fit(train_ds,
                           ...
                           callbacks=callbacks,
                           verbose=1)
            # Sauvegarder l'historique phase 1
            hist_p1 = pd.DataFrame(h1.history)
            hist_p1.to_csv(os.path.join(args.output_dir, 'history_phase1.csv'), index=False)
            # Sauvegarder l'état
            with open(state_path, 'w') as f:
                json.dump({'phase': 1, 'epoch': len(hist_p1)}, f)
        else:
            print("[INFO] Stage 1: Skipped (already completed)")
            h1 = type('obj', (object,), {'history': hist_p1.to_dict('list') if hist_p1 is not None else {}})()
        # Phase 2: fine‑tune top layers
        print(f"[INFO] Stage 2: Fine-tune top layers - Starting from epoch {initial_epoch_p2}")
        ...
        # Sauvegarder l'état
        with open(state_path, 'w') as f:
            json.dump({'phase': 2, 'epoch': initial_epoch_p2 + len(h2.history.get('loss', []))}, f)
