...
def main():
    ...
    if args.report_only:
        ...
        h1 = model.fit(train_ds,
                       ...
                       callbacks=callbacks,
                       verbose=1)
        # Phase 2: fine‑tune top layers avec warm-up progressif
        print("[INFO] Stage 2: Fine-tune top layers (with LR warm-up)")
        unfreeze_top_layers(base, fine_tune_at=args.fine_tune_at)
        
        # Scheduler avec warm-up progressif pour éviter la divergence
        initial_ft_lr = args.ft_lr / 10.0  # Démarrer 10x plus bas
        def lr_schedule_warmup(epoch):
            warmup_epochs = 3
            if epoch < warmup_epochs:
                # Warm-up linéaire de ft_lr/10 vers ft_lr
                return initial_ft_lr + (args.ft_lr - initial_ft_lr) * (epoch / warmup_epochs)
            else:
                # Décroissance exponentielle douce après warm-up
                return args.ft_lr * (0.95 ** (epoch - warmup_epochs))
        
        try:
            opt_ft = keras.optimizers.AdamW(learning_rate=initial_ft_lr, weight_decay=args.weight_decay)
        except Exception:
            opt_ft = keras.optimizers.Adam(learning_rate=initial_ft_lr)
        model.compile(optimizer=opt_ft, loss=loss_dict, metrics=metrics_dict, loss_weights=loss_w)
        
        # Ajouter le LR scheduler
        lr_callback = keras.callbacks.LearningRateScheduler(lr_schedule_warmup, verbose=1)
        callbacks_ft = callbacks + [lr_callback]
