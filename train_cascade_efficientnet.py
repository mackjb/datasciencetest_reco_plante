...
# -*- coding: utf-8 -*-
"""
Entraînement en cascade avec EfficientNetV2 pour PlantVillage (2 modèles):

ENTRAÎNEMENT:
  1) Modèle ESPÈCE: EfficientNetV2B0 (fine-tuning 2 phases)
     - Input: Image
     - Output: Espèce (14 classes)
  
  2) Modèle MALADIE GLOBAL: EfficientNetV2B2 + Attention (fine-tuning 2 phases)
...
VERSION: EfficientNetV2 (pour comparaison avec DenseNet121/ResNet50V2)
"""
import argparse
import json
import os
...
def make_dataset_species(paths, labels, img_size=(256, 256), batch_size=64, 
    ...
    """Crée un tf.data.Dataset pour le modèle d'espèce (EfficientNetV2B0, pas d'augmentation)."""
...
def make_dataset_disease(paths, species_labels, disease_labels, img_size=(256, 256), batch_size=64,
    ...
    """Crée un tf.data.Dataset pour le modèle de maladie (EfficientNetV2B2+Attention, 2 entrées: image + espèce)."""
    AUTOTUNE = tf.data.AUTOTUNE
...
def build_species_model_efficientnetv2(num_classes, img_size=(256, 256), 
                               initial_lr=1e-3, weight_decay=1e-4):
    """Construit un modèle EfficientNetV2B0 pour l'espèce."""
    ...
    base._name = 'efficientnetv2b0_backbone'  # Renommer après création
...
def build_disease_model_efficientnetv2_attention(num_diseases, num_species, img_size=(256, 256), 
                                        initial_lr=1e-3, weight_decay=1e-4):
    """Construit EfficientNetV2B2 + Attention pour la maladie, conditionné sur l'espèce."""
    # Entrée image
    ...
    base = EfficientNetV2B2(include_top=False, weights='imagenet', input_shape=(*img_size, 3))
    base.trainable = False  # Phase 1: gelé
    base._name = 'efficientnetv2b2_backbone'  # Renommer après création
...
def train_species_model(train_paths, train_species_labels,
    ...
    print("\n" + "="*60)
    print("ÉTAPE 1: Modèle ESPÈCE (EfficientNetV2B0, 2 phases)")
    print("="*60)
    ...
    model = build_species_model_efficientnetv2(num_species, tuple(args.img_size),
    ...
    print(f"\n[PHASE 2] Fine-tuning EfficientNetV2B0 backbone ({args.epochs_phase2} époques)")
    efficientnet_backbone = model.get_layer('efficientnetv2-b0')
    for layer in efficientnet_backbone.layers[-50:]:
...
def train_disease_model_global(train_paths, train_species_labels, train_disease_labels,
    ...
    print("\n" + "="*60)
    print("ÉTAPE 2: Modèle MALADIE GLOBAL (EfficientNetV2B2+Attention)")
    ...
    # Construire modèle EfficientNetV2B2 + Attention
    model = build_disease_model_efficientnetv2_attention(num_diseases, num_species,
                                                         tuple(args.img_size),
                                                         args.initial_lr, args.weight_decay)
    ...
    # PHASE 2: Fine-tuning (dégel des 100 dernières couches du EfficientNetV2B2)
    print(f"\n[PHASE 2] Fine-tuning EfficientNetV2B2 ({args.epochs_disease_p2} époques)")
    efficientnet_backbone = model.get_layer('efficientnetv2-b2')
    for layer in efficientnet_backbone.layers[-100:]:
        layer.trainable = True
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='outputs/cascade_efficientnet',
